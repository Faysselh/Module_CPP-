...
void	Bureaucrat::signForm(Form &form)
{
	try
    {
        if (this->getGrade() > form.getGradeSign())
            throw   Bureaucrat::GradeTooLowException();
        form.beSigned(*this);
        std::cout << this->getName() << " signe " << form.getName() << std::endl;
    }
    catch(const std::exception& e)
    {
        std::cerr << this->getName() << " cannot sign the document ";
		std::cerr << form.getName() << " because " << this->getName() << " " << e.what() << std::endl;
    }
}